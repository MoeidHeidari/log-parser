...
export class FileHelper<T> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        reader.on('line', async (line: string) => {
          try {
            const log = await this.parsLog(line, filter);
  ...
  async parsLog(log: string, filter: string[]): Promise<T | any> {
    try {
      const log_level: string = log
        .match('\\s-\\s\\w+\\s-\\s')[0]
        .replace(/\s-\s/g, '');
      if (!filter.includes(log_level) && filter.length > 0) return;
      const time_stamp = new Date(
        log.match(
          '[0-9]{1,4}-[0-9]{1,2}-[0-9]{1,2}[A-Z][0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}.[0-9]{1,3}[A-Z]',
        )?.[0],
      ).getTime();
      const log_info = JSON.parse(log.match('{(?:)(.*)}')[0]);
      const transaction_id = log_info.transactionId;
      const err = log_info.details;
      return {
