...
describe('tests the behavior of parser service class', () => {
  ...
  it(PARSER_INPUT_TEST_CASE_1.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_1.data),
    );
  ...
  it(PARSER_INPUT_TEST_CASE_2.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_2.data),
    );
    expect(
      (
        await result.filter(
          (log: { logLevel: string }) => log.logLevel === 'debug',
        )
      ).length,
    ).toBeGreaterThan(0);
    expect(
      (
        await result.filter(
          (log: { logLevel: string }) => log.logLevel === 'error',
        )
      ).length,
    ).toBeGreaterThan(0);
  });
  ...
  it(PARSER_INPUT_TEST_CASE_3.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_3.data),
    );
  ...
  it(PARSER_INPUT_TEST_CASE_4.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_4.data),
    );
    expect(
      (
        await result.filter(
          (log: { logLevel: string }) => log.logLevel === 'warn',
        )
      ).length,
    ).toBeGreaterThan(0);
    expect(
  ...
  it(PARSER_INPUT_TEST_CASE_5.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_5.data),
    );
    expect(
      (
        await result.filter(
    ...
    expect(
      (
        await result.filter(
          (log: { logLevel: string }) => log.logLevel === 'info',
        )
      ).length,
    ).toBeGreaterThan(0);
  });
  ...
  it(PARSER_INPUT_TEST_CASE_6.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_6.data),
    );
    expect(
      (
        await result.filter(
  ...
  it(PARSER_INPUT_TEST_CASE_8.expectation.should, async () => {
    const result = await service.parse(
      new ParserCommandDTO(PARSER_INPUT_TEST_CASE_8.data),
    );
    expect(
      (
        await result.filter(
